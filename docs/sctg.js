  ...
  var compute = function (data, config) {
    ...
    var setBottomNodeLayout = function (items) {
      const itemsWithLayout = items.map(d => {
        ...
        d = { ...d }; // Always return a copy don't modify original object
        d.layout = layout;
        return d;
      });
    ...
    d.value.items.map(item => {
      return {
        source: { x: item.layout.x, y: item.layout.y },
        target: { x: d.value.layout.x, y: d.value.layout.y } };
...
function draw(sctg_layout_values, svg, logDiv, config, style_config) {
  ...
  data(d => {
    return d.value.items.map(item => {
      return {
        source: { x: d.value.layout.x, y: d.value.layout.y },
        target: { x: item.layout.x, y: item.layout.y } };
  ...
  call(
  d3.
  axisLeft(sctg_layout_values.scales.bottom_y_scale).
  ...
  call(
  d3.
  axisRight(sctg_layout_values.scales.bottom_y_scale).
  tickFormat(d3.format(".0s")));
  ...
  call(
  d3.
  axisLeft(sctg_layout_values.scales.top_y_scale).
  tickFormat(d3.format(".0s")));
  ...
  call(
  d3.
  axisRight(sctg_layout_values.scales.top_y_scale).
  tickFormat(d3.format(".0s")));
